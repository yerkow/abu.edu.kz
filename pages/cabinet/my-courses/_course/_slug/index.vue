              ...
              <div class="row">
                              ...
                              <el-tab-pane :label="index+1+' бөлім'" :class="index==0?'active':''" :name="'section'+index" v-for="(section,key,index) in course.sections" :key="index">
                                <table class="table">
                                  ...
                                  <tr v-for="(lesson,lessonIndex) in course.sections[key]" :key="lesson.id"
                                      :class="lessonIndex==0&&index==0?'warning':''">
                ...
                <div class="col-md-3">
                  ...
                  <aside class="mu-sidebar">
                    ...
                    <!-- end single sidebar -->
                    <div class="mu-single-sidebar">
                      <nuxt-link :to="$store.getters['auth/isAuthenticated']?'/testing/start':'/register?message=unt'">
                        <img src="/img/go_test.gif" alt="ҰБТ тест тапсыру тегін"
...
<script>
  ...
  export default {
    ...
    async asyncData({store, params}) {
      const course = await store.dispatch('courses/fetchLessonsBySlug', params.slug);
      const title = course.courseOverviewTitle
      return {course, title}
    },
    data() {
      return {
